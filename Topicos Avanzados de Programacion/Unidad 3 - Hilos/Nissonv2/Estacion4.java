...
public class Estacion4 extends Thread {
	...
	private Estacion5 estacionSiguiente;
	private Vista vista;
	private int linea;

	public Estacion4(int linea, Semaforo sEntrada, Vista vista) {
		this.linea = linea;
		...
		estacionSiguiente = new Estacion5(linea, sSalida, vista);
	...
	public void run() {
		while (true) {
			...
			sRobots.Espera();
			System.out.println("Linea #" + (linea+1) + ", Robot trabajando en la E4 ");
			vista.trabajar(linea, 4, true);
			dormir(500);
			vista.trabajar(linea, 4, false);
			sRobots.Libera();
