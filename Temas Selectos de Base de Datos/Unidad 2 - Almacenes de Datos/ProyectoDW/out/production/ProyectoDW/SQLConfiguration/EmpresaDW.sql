...
CREATE TABLE Dimension.Fecha(
   ...
   Cuatrimestre NVARCHAR(15) NOT NULL,
   Trimestre NVARCHAR(15) NOT NULL,
   Bimestre NVARCHAR(15) NOT NULL,
   Estacion NVARCHAR(15) NOT NULL,
   SemanaDelAnio NVARCHAR(15) NOT NULL,
   NombreDiaSemana NVARCHAR(15) NOT NULL,
   DiaDelAnio NVARCHAR(15) NOT NULL
)
GO
-- Procedimiento Almacenado para llenar Fecha
CREATE PROCEDURE InsertarFecha @FechaActual datetime
AS
INSERT INTO Dimension.Fecha(
	...
	(DATEPART(year , @FechaActual) * 10000) + (DATEPART(month , @FechaActual)*100)
	   ...
	   CASE WHEN MONTH(@FechaActual) <= 6 THEN 'Primer Semestre' ELSE 'Segundo Semestre' END,
	   CASE WHEN MONTH(@FechaActual) BETWEEN 1 AND 4 THEN 'Cuatrimestre 1'
            WHEN MONTH(@FechaActual) BETWEEN 5 AND 8 THEN 'Cuatrimestre 2'
            WHEN MONTH(@FechaActual) BETWEEN 9 AND 12 THEN 'Cuatrimestre 3' END, /* Cuatrimestre */
       CASE WHEN MONTH(@FechaActual) BETWEEN 1 AND 3 THEN 'Trimestre 1'
            WHEN MONTH(@FechaActual) BETWEEN 4 AND 6 THEN 'Trimestre 2'
            WHEN MONTH(@FechaActual) BETWEEN 7 AND 9 THEN 'Trimestre 3'
            WHEN MONTH(@FechaActual) BETWEEN 10 AND 12 THEN 'Trimestre 4' END, /* Trimestre */
       CASE WHEN MONTH(@FechaActual) BETWEEN 1 AND 2 THEN 'Bimestre 1'
            WHEN MONTH(@FechaActual) BETWEEN 3 AND 4 THEN 'Bimestre 2'
            WHEN MONTH(@FechaActual) BETWEEN 5 AND 6 THEN 'Bimestre 3'
            WHEN MONTH(@FechaActual) BETWEEN 7 AND 8 THEN 'Bimestre 4'
            WHEN MONTH(@FechaActual) BETWEEN 9 AND 10 THEN 'Bimestre 5'
            WHEN MONTH(@FechaActual) BETWEEN 11 AND 12 THEN 'Bimestre 6' END, /* Bimestre */
